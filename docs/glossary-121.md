...
| Term | Definition |
| :------------------ | :------------------------------------------------------------------- |
| **121 platform** | (“one to one”) is a platform that helps program managers by increasing efficiency in setting up and executing a cash program Information Management (IM) system. |
| **CVA** | Cash and Voucher Assistance, referring to Cash Transfer Programming (CTP). |
| **Program** | A program created by users in the platform. |
| **FSP** | Financial Service Provider - an entity that provides financial services, which may include digital payment services. |
| **Budget** | Amount of money in the program that will be disbursed to included people/households. |
...
| **Quick search** | A filter search that shows results of multiple columns instead of filtering in just one column. This is a fast way to find a single or small group of registrations. |
...
| **Payment or payments** | A collection of transfers to one or multiple registrations. |
| **Transfer** | A single transfer of money to a registration. A registration will receive the number of transfers pre-decided in the program. Depending on the program, if registered late, the first transfer to the registration might be part of the third payment of a program. |
...
| Term | Definition |
| :------------------ | :------------------------------------------------------------------- |
| **All programs** | The home page of 121 contains all programs, running and archived, that the users are assigned to. |
| **Users** | All individuals who have access to, and work on the 121 platform e.g. CVA Officer, volunteers, Finance Officers etc. A user can have different roles in different programs. |
| **Roles** | Each user will be assigned a position for a specific program (e.g. CVA Officer, volunteer, Finance Officer etc.). This is done in the "Team" page of the program details. Each role has 1 or more permissions. |
| **Permissions** | Each role has different authorization to perform different actions. e.g. only a Finance Officer can approve a payment. You can see which permissions are related to which role on the "Roles and permissions" page. |
...
| Term | Definition |
| :------------------ | :------------------------------------------------------------------- |
| **Registrations target** | The amount of people/households that a program wishes to help. |
| **Registrations reached** | The amount of registrations that have been receiving funds through a program. |
| **Budget** | The amount of funds allocated for disbursement in a program. |
| **Cash disbursed** | The amount of funds transferred to the registrations in a program. |
| **Last payment** | The date in which the last payment was made. |
| **Active** | Programs that are being created or already running. |
| **Archived** | Rather than deleting, the user can archive a program; it is not active but its details are still available to view and can be copied. |

### Create new program

| Term | Definition |
| :------------------ | :------------------------------------------------------------------- |
| **Program details** | In this page users can fill in the information of the program like its name, amount of registrations the program wishes to reach etc. |
| **Program title** | Where the users can rename the program. |
| **Data mapping** | Sync kobo/csv file into the data table you'll be running the program on. |
...
| **Completed** | The registration received the full amount of transfers. |
| **Deleted** | The registration has been deleted, their personal details were deleted - only some details are still available for reporting. |
| **Paused** | The registration is currently not being included in the payments but is still part of the program. |
...
| **Transfer value** | The amount of money a registration is meant to receive in a single payment. |
| **Transfer** | A single transfer of money to a registration. A registration will receive the number of transfers pre-configured in the program. If registered late, the first transfer to a registration might be part of a later payment tranche. |
| **Total payment amount** | The sum value of adding up the transfer values of each included registration added to the payment. |
| **Waiting for approval** | A new payment has been created and it needs to be approved by a Finance Officer. |
