...
??? "1. Comment les données sont-elles protégées ? Quelle est la politique de protection des données ?"
      ...
      510, l'unité numérique et des données, fait partie de la Croix-Rouge néerlandaise (CRNL). La CRNL a une **politique de protection des données** en vigueur depuis 2018. **La CRNL et la plateforme 121 adhèrent au Règlement général sur la protection des données (RGPD) européen**. Le RGPD a également spécifié la signalement de violation de données, qui doit être signalée à l'Autorité néerlandaise Persoonsgegevens. La CRNL emploie un agent juridique spécialisé dans la protection des données. La CRNL est un processeur de données pour le partenaire, et ceci est convenu dans l'accord de niveau de service (SLA). Pour plus d'informations sur le processeur de données : https://www.gdpreu.org/the-regulation/key-concepts/data-controllers-and-processors/ 
